...
import me.craftymcfish.nomorehorses.block.custom.entity.MaceratorBlockEntity;
...
public class FisherBlock extends BlockWithEntity implements BlockEntityProvider {
    private static final VoxelShape SHAPE = FisherBlock.createCuboidShape(0, 0, 0, 16, 16, 16);
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        return (BlockState)this.getDefaultState()
                .with(Properties.HORIZONTAL_FACING, ctx.getHorizontalPlayerFacing().rotateClockwise(Direction.Axis.Y))
    ...
    public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (blockEntity instanceof FisherBlockEntity) {
                ItemScatterer.spawn(world, pos, (FisherBlockEntity)blockEntity);
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
        return validateTicker(type, ModBlockEntities.FISHER_BLOCK_ENTITY,
